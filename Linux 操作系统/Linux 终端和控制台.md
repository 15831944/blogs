...
1. 终端是一种字符型设备，它有多种类型，通常使用 tty 来简称各种类型的终端设备。
2. tty 是 Teletype 的缩写。
3. Teletype 是最早出现的一种终端设备，很像电传打字机，是由Teletype公司生产的。
4. 设备文件放在特殊文件目录/dev/下。
...
# 2.3 控制台终端（/dev/ttyn, /dev/console）
