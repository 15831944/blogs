...
// 基类的窗口过程函数, 对消息进行响应 wanghuiyong
LRESULT CWnd::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	// OnWndMsg does most of the work, except for DefWindowProc call
	LRESULT lResult = 0;
	if (!OnWndMsg(message, wParam, lParam, &lResult))		// 真正完成消息路由
		lResult = DefWindowProc(message, wParam, lParam);
	return lResult;
}

// 完成消息路由, 相当于SDK程序中的消息循环, 消息映射的处理, 判断该消息有没有我们编写的消息响应函数 wanghuiyong
...
// 第 11 步: 设计窗口类(由各种窗口类的创建窗口的函数调用) wanghuiyong
BOOL AFXAPI AfxEndDeferRegisterClass(LONG fToRegister)	// AfxDeferRegisterClass
{
