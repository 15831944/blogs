...
void CDocManager::OnFileNew()
{
	...
	// 对于单文档程序, 只调用了一次 AddDocTemplate 函数, 只有一个文档模板, 所以不会进入此判断
...
CDocument* CDocManager::OpenDocumentFile(LPCTSTR lpszFileName, BOOL bAddToMRU)
{
	...
	// 保存后打开同一个文件, pOpenDocument 关联先前的文档, 就不为空, 打开新文件才会先清空 wanghuiyong
	if (pOpenDocument != NULL)
	{
		...
		return pOpenDocument;		// 在这里返回就不会执行 Serialize 函数了 wanghuiyong
	}
