...
CDocument* CSingleDocTemplate::OpenDocumentFile(LPCTSTR lpszPathName, BOOL bAddToMRU, BOOL bMakeVisible)
{
	...
	else
	{
		...
		pDocument = CreateNewDocument();	// 创建文档对象, 单文档应用程序会重复的利用这里创建的对象 wanghuiyong
	...
	if (pFrame == NULL)
	{
		...
		pFrame = CreateNewFrame(pDocument, NULL);	// 创建框架对象, 同时创建视图类对象
	...
	if (lpszPathName == NULL)
	{
		...
		if (!pDocument->OnNewDocument())	// 会调用到向导生成的子类中的同名函数 wanghuiyong
		{
