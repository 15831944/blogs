...
{
	// 从文档类获取视图类(从视图类获取文档类更为简单)
	// POSITION pos = GetFirstViewPosition();
	// CTestView *pView = (CTestView *)GetNextView(pos);
	// pView->m_obArray.Serialize(ar);		// 直接使用对象数组的串行化功能
...
void CTestDoc::DeleteContents()
{
	...
	for (int i = 0; i < nCount; i++)
	{
		delete m_obArray.GetAt(i);	// 销毁 pGraph 对象
		// m_obArray.RemoveAt(i);		// 移除元素之后数组大小会发生变化, 循环会出错, 导致内存泄漏
	}
	m_obArray.RemoveAll();
