...
public:
	Remote(int m = TV) : mode(m) {}		// 自定义默认构造函数, 默认参数值初始化为TV调谐模式
	...
	void	chanup(Tv & t);
	void	chandown(Tv & t);
	void	set_chan(Tv & t, int c);		// 友元成员函数, 访问了Tv的私有数据成员
	void	set_mode(Tv & t);
	void	set_input(Tv & t);
};
...
{
public:
	// 声明友元成员函数(仅让类的特定成员成为友元)
	friend void Remote::set_chan(Tv & t, int c);	// Remote 类的声明和此成员函数的声明必须放在友元成员函数的前面
