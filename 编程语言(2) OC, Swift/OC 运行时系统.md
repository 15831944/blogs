...
1. 方法覆写(override): 方法的名称, 参数和返回值类型均相同, 使用 super 将调用(直系)父类方法, 使用 self 将调用当前类方法(相当于递归调用), 使用父类指针指向子类对象, 将调用子类的重写的方法, 在分类中覆写方法会导致原方法无法访问(隐藏)
2. 方法重载(overload): 方法名相同的函数互为重载, OC 不支持方法重载, 因为方法的返回值类型和参数类型需要是静态类型, 才能查找到正确的方法

# 事件及响应链 responder chain

1. UIResponder 为处理响应事件和响应链定义了界面和默认行为, 当用户用手指滚动列表或者在虚拟键盘上输入时, UIKit 就生成时间传送给 UIResponder 响应链, 直到链中有对象处理这个事件

1. 第一反应者(first responder):
2. 事件处理(event-handling):

# 消息发送和转发 Message Sending and Forwarding

1. 消息的发送: 将消息发送给没有实现该方法的对象时, 通常会出现运行时错误, 此时可以进行转发

通过 SEL 类型来指定要执行的方法, 这就是 OC 发送消息的方式, 以实现 OC 的动态性

forwardInvocation:	// 接受者没有实现应对某消息的方法, 运行时系统会发送此消息给接受者, 在 NSObject 中定义, 由接收者重写, 允许一个对象将消息转发给另一个对象
...
void objectMessageSend(id self, SEL cmd, ...);	//  C 函数形式: self 是消息接收者, cmd 内置变量是选择子, 始终包含当前方法的选择器, 可变参数是消息的参数
```
