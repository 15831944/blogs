...
3. switch 语句支持整型, 字符型
4. 一些应用程序为了优化要求使用底层方法, 比如使用大型的数据数组, 可能会使用 C 语言的内置数据结构, 而不是 Foundation 的数组对象
5. 使用函数指针作为参数的函数有 qsort, 方法有 sortUsingFoundation:context, 可对任意数组进行排序
6. 高阶函数还可以用于分派表 13.5.4
7. 复合字面量: (type) {initialize list}, 可以用于初始化结构或数组
8. 所有的运算符都产生一个值
9. main 函数的命令行参数: argc, argument count; argv, argument vector, 可使用 NSProcessInfo 类访问命令行参数

# 本质

1. 对象是结构, 对象的实例变量是结构的成员变量, 继承的实例变量和自身的变量组成了新的结构
2. 对象变量是指向结构的指针, 因此才可以用"->"运算符访问 `@public` 成员(不推荐)
3. 方法是函数, 消息表达式是函数调用, 调用方法时, 是在调用与接收者相关的函数, 传递给函数的参数是接收者(self)和方法的参数
4. id 类型是通用指针类型, id 类型的方法只是返回指向内存中某个对象的指针, 对象总是携带 isa 成员, 因此总是可以确定它的类
...
20. 函数的默认返回类型为整型, 方法的默认返回类型为 id 类型, 推荐显式声明返回类型

# 块 blocks

1. 块是 Apple 对 C 语言的扩展, 看起来像函数, 可以传参, 具有返回值, 类似于 swift 中的闭包
2. 块定义在函数或者方法内部, 并能够访问在函数或者方法范围内块之外的任何变量
...
1. 模块(module): 引用包含在一个源文件中任何数目的方法或者函数定义, 为系统头文件提供更快的处理速度(重新编译它们)和避免了潜在的命名冲突, `#import UIKit`
1. 将类的定义模块化到相关的方法的组或分类中, 是更简便的扩展类的方式, 无须访问类的源代码, 无须创建子类
1. 分类可以访问主类的实例变量和方法, 不能在分类中列出父类或实例变量
1. 分类的文件名称习惯为: 主类名接分类名
1. 分类的接口可以定义在单独的接口文件中(应包含主类的头文件)或写在主类接口文件下面(前提是可以访问主类的源代码)
1. 分类的实现可以定义在单独的实现文件中(主类不能访问分类方法)或写在主类实现文件下面(该类的所有用于都可以访问分类中的方法)
1. 未命名的分类: 用于类的扩展
1. Objective-C 名称空间是程序代码与所有的库, 框架和插件共享的
...
1. 协议是多个类共享的一个方法列表, 是无类的(classless),  不引用任何类
2. 协议是两个类之间的接口定义, 定义了协议的类可以看作是将协议定义的方法代理给了实现他们的类
2. 遵守(confirm to)或采用(adopt)协议表示实现该协议的所有方法
3. 实现的协议的方法可以被继承
4. 协议的扩展
5. 分类也可以采用协议
...
非正式(informal)协议, 实际上是一个分类, 通常用于根类, 也成为抽象(abstract)协议
非正式协议实际上仅仅是一个名称之下的一组方法, 声明非正式协议的类自己并不实现这些方法, 选择实现这些方法的子类需要在它的接口部分重新声明这些方法, 并实现这些方法中的一个或多个
编译器不提供有关非正式协议的帮助, 没有遵守协议或者由编译器测试这样的概念

# 合成对象

我感觉相当于 C++ 的类的组合, 有一个的关系

# 手工引用计数和自动引用计数(ARC)

1. 手工引用计数需要给对象发送 release 消息, 来释放对象
2. 如果使用自动引用计数, 那么每次调用函数(方法)时, 局部对象的变阿玲都会默认初始化为空 13.2.1
