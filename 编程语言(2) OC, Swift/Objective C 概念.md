...
3. 分类可以访问主类的实例变量和方法, 不能在分类中列出父类或实例变量
4. 分类的文件名称习惯为: 主类名接分类名
5. 分类的接口可以定义在单独的接口文件中(应包含主类的头文件)或写在主类接口文件下面(前提是可以访问主类的源代码)
6. 分类的实现可以定义在单独的实现文件中(主类不能访问分类方法)或写在主类实现文件下面(该类的所有用于都可以访问分类中的方法)
7. 未命名的分类: 用于类的扩展
...
2. 定义了协议的类可以看作是将协议定义的方法代理给了实现他们的类
